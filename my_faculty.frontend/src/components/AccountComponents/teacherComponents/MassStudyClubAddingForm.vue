	...
	<v-row justify="center">
		...
		>
			<v-card>
				<v-card-title>
					<span>
						Массовое добавление студентов в сообщество
					</span>
					<v-spacer></v-spacer>
					<v-btn
						icon
				...
				<v-card-text>
					<v-container>
						<h3 class="black--text">
							В данном разделе Вы можете добавлять массово студентов в свое
							сообщество курса, выбрав добавление либо определенной группы студентов, либо целого
							курса (будут добавлены те пользователи, в чьих профилях указаны соответствующие
							курс / группа).
						</h3>
						<v-form
							lazy-validation
						...
						>
							...
							<h3 v-else-if="groupsAreLoaded === false" class="red--text">
								...
								Возможно только добавление всего курса
							</h3>
						</v-form>
					</v-container>
	...
	methods: {
		...
		generateButtonText() {
			...
			if (this.selectedCourseId !== null && this.selectedGroupId === null)
				return 'Добавить студентов из выбранного курса';
			return 'Добавить студентов из выбранной группы';
		},
		loadCoursesList() {
			this.$loading(true);
		...
		addCourse() {
			this.$loading(true);
			this.$store.dispatch('addUsersFromCourseToStudyClub', {
		...
		addGroup() {
			this.$loading(true);
			this.$store.dispatch('addUsersFromGroupToStudyClub', {
				issuerId: this.$oidc.currentUserId,
				studyClubId: this.studyClubId,
